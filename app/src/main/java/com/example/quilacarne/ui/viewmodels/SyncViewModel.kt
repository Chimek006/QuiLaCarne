    ...
    fun startSync() {
        viewModelScope.launch {
            ...
            repository.getBootstrap().fold(
                    ...
                    try {
                        ...
                        if (totalOrderPages > 0) {
                            for (page in 1..totalOrderPages) {
                                ...
                                repository.getOrdersSync(page).onSuccess { ordersDto: List<OrderSyncDto> ->
                                        ...
                                        try {
                                            OrderEntity(
                                                ...
                                                statusId = dto.statusTokens.firstOrNull()?.let {
                                                    try { UUID.fromString(it) } catch(e: Exception) { UUID.randomUUID() }
                                                } ?: UUID.randomUUID(),
                        ...
                        if (totalItemPages > 0) {
                            for (page in 1..totalItemPages) {
                                ...
                                _uiState.value = SyncUiState.Loading("Pobieranie dań (strona $page/$totalItemPages)", progress)

                                repository.getOrderItemsSync(page).onSuccess { itemsDto: List<OrderItemSyncDto> ->
                ...
                onFailure = { error ->
                    _uiState.value = SyncUiState.Error("Błąd sieci: ${error.message}")
                }
            )
        }
    }
